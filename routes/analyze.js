...
// }
//
// // Analyze Grain Size using a simplified circular intercept method
...
//
//                 let intercepts = 0;
//                 const numSamples = 36; // Number of points to sample on the ellipse
//                 const rayLength = Math.max(radiusX, radiusY) * 0.2; // Length of the rays
//                 const numRays = 4; // Number of rays per sample point
//                 const intensityThreshold = 20; // Threshold for intensity change
//
...
//
//                         // Sample intensities along the ray (simplified - just start and end)
//                         const startIntensity = await getPixelIntensity(preprocessedBuffer, sampleX, sampleY);
//                         const endIntensity = await getPixelIntensity(preprocessedBuffer, endX, endY);
//
//                         if (!isNaN(startIntensity) && !isNaN(endIntensity) && Math.abs(startIntensity - endIntensity) > intensityThreshold) {
//                             intercepts++;
//                         }
//                     }
//                 }
//
//                 // Estimate average grain size (simplified - proportional to perimeter/intercepts)
...
//                     averageGrainSize = scale ? averageInterceptDistance * scale : averageInterceptDistance;
//                 }
//
//                 analysisResults.push({
//                     id: shape.id,
...
//     } catch (error) {
//         console.error('Error during grain size analysis (circular intercept attempt):', error);
